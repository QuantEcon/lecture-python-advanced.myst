...
This can be written as a standard linear state space system.
...
However, we will use a different set of code for simulation, for reasons described below.
...
Let the increment in $\{y_t\}$ obey the following equation.
...
Notice the persistent but irregular growth in $y_t$.
...
To attain this decomposition for the particular class of additive functionals defined by {eq}`old1_additive_functionals` and {eq}`old2_additive_functionals`, we first construct the following matrices.
...
This approach allows us to use the routines in [LinearStateSpace](https://github.com/QuantEcon/QuantEcon.py/blob/master/quantecon/lss.py) to study dynamics.

To start, observe that under the dynamics in {eq}`old1_additive_functionals` and {eq}`old2_additive_functionals` and with the definitions just given, the following relationship holds.
...
A hint that it does more is the name of the class: AMF stands for "additive and multiplicative functional."
...
If you run {ref}`the code that first simulated that example <addfunc_egcode>` again and then the method call, you will generate the following plot (modulo randomness).
...
As we have seen, it has the following representation.
...
The following code adds some simple functions.

These functions make it straightforward to generate sample paths from an instance of `AMF_LSS_VAR`.
